...
<fwa-section-light>
  <fwa-container-full>
    ...
    <p class="mb6 text-2 text1-lg leading4">Logical architecture is the shape of the system source code. Physical architecture is the shape of the cloud infrastructure running the source code. The distinction is authortime vs runtime. The ideal would be there is no distinction between the code we author and the code that runs. This was not the case in the early days of running MVC applications on the cloud. In order to meet demand and remain fault tolerant, at a minimum, the logical architecture runs on three physical web servers, behind a load balancer, which itself is ideally behind a CDN, all talking to one or more database servers and, quite often also a caching server. Worse, often in the early days, the infrastructure definition would live separate from the code. This was a mistake because it ultimately means the system is not consistently reproducible which makes bug resolution extremely slow at best, and impossible at worst. Today most MVC systems have evolved to define infrastructure alongside the code that depends on it (often called Infrastructure as Code or IaC). </p>

    <fwa-sub-heading class="block mb3">
      JAMstack architecture
    </fwa-sub-heading>
    <p class="mb6 text-2 text1-lg leading4">One way to deal with all this complexity is to return to a two-tier architecture. JAMstack is a modern “thick client” version of two-tier architecture. Pre-rendered markup for the presentation layer, and JavaScript for application logic make up the client. APIs act as the server. Like all architectures, JAMstack comes with tradeoffs. Dynamic data and personalization functionality will be trapped behind a loading spinner instead of being immediately available because the application logic relies on the browser client JavaScript. Another issue is that most implementations of the JAMstack physical architecture fall prey to the same mistake made in the early days of MVC on the cloud. Even if the application logic is immutably deployed none of the API service infrastructure is determinstically reproducible. The JAMstack physical architecture relys on outsourcing to third party API providers for dynamic functionality. As a result, key infrastructure becomes spread across services making deterministic deployment, and governance more complex.</p>
